...
- B10 ✅ 35 tests verdes + `npm run build` OK + smoke E2E completo
- B11 ✅ docs/backend-criteria.md (resultados, desempate, horarios, brackets, zonas, auth)
- ✅ CERRADO — commit único back: 224dc4e feat(back): services, auth, SSE and API surface (B4/B7/B8/B9/B11). Sin push (pendiente decisión del usuario).
- ADDENDUM 2026-09-23 (desempate, patch): H2H pasa a 2º criterio intra-zona (usa winnerId — WINNER_ONLY alcanza), luego setDiff, luego sorteo determinístico (nombre, id); ciclo H2H ya no bloquea. Mejores segundos (3 zonas): empate exacto de 2 → se crea Match stage DESEMPATE (entra en cadena de estimados, slot tras grupos, corrida de slots de llaves +1; phase DESEMPATE; semis esperan su resultado; al cargarlo → ELIMINATORIES). Borde 3+ segundos → detección y reporte sin romper flujo (SSE brackets-blocked + /api/state.bracketsBlocked). /api/state expone desempate {needed,pending,match}. nextMatch ignora slots sin equipos. SQLite: enums Prisma = TEXT sin CHECK → agregar valor enum NO genera migración (cliente valida); restart dev tras generate. 41 tests verdes (+6).