...
- The rest of the app (SSE, auth, editing guards) is etapa-agnostic but must
  ...
  como S5 pendiente. Gotcha: `createEtapa` devolv�a `matchCount`
  teams*(teams-1)+3 ignorando la zonificaci�n (ej 6 equipos dec�a 33, real 9)
  � corregido al tama�o real de fixture.
  Checks: 140/140 tests, tsc, lint. Smoke seed OK + /api/state 200.
- 2026-09-26 User: subi� 2 archivos m�s de etapas jugadas (las faltantes se
  suspendieron) y va a dictar qui�n es hombre/mujer de los nombres � NO
  inferir g�nero; solicitar la lista cuando se procesen los briefs.
...
- Aliases (user-confirmed): Sebita = Seba (same player); Santy = Santi (same
  player); Gus != Gastón; Gabi sz (E3, with Cami) != Gabi (E5, with Nabi);
  Cami (E3, with Gabi sz) != Cami C (E3, with Deivid/David).
- `prisma/seed.ts` now loads THREE etapas idempotently:
  - Etapa 1 (REPECHAJE): 6 teams (A: Roxi y Gastón, Ángela y Migue, Seba y
    Sil; B: Mati y Cin, Kari y Santi, Vivi y Andrés). 6 group matches with
...
- [x] (done) data: user dictation per-sex + alias normalization (Sebita/Seba,
  Santy/Santi, Gus/Gastón, Gabi sz/Gabi, Cami C/Cami)
...
Tasks:
- [x] schema: Etapa.bracketFormat enum (STANDARD default | REPECHAJE) + migration ADD COLUMN — `20260925140000_etapa_bracket_format` applied to dev.db (2026-09-26)
...
- [x] ranking computeFinalPositions: BRONZE winner/loser = 3�/4� when present (fallback stays)
